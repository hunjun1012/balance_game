...
<body onload="InitializeStaticMenu();">
  <div class="wrap">
    ...
    <section id="lovetest" style="margin: 20px;">
      <center>
        <p style="font-size: 30px;">나의 100점짜리<br> 아내 만들기</p><br /><br />
      </center>
      <!-- 1. 재력 -->
      <div>
        <p style="font-size: 20px;">1. 경제관념</p>
        <input name="product" id="check1" value="30" type="checkbox" onclick="totalIt()" /><label for="check1">1) 꼼꼼하고 나름 재테크를 잘함 -30점</label>
        <br>
        <input name="product" id="check1" value="25" type="checkbox" onclick="totalIt()" /><label for="check1">2) 저축은 하지만 돈 잘 빌려줌 -25점</label>
        <br>
        <input name="product" id="check1" value="20" type="checkbox" onclick="totalIt()" /><label for="check1">3) 월급이 잠깐 쉬었다 감 (빚은 없음) -20점</label>
        <br>
        <input name="product" id="check1" value="15" type="checkbox" onclick="totalIt()" /><label for="check1">4) 할부인생(매월 할부금이 늘어남) -15점</label>
        <br>
        <input name="product" id="check1" value="10" type="checkbox" onclick="totalIt()" /><label for="check1">5) 사채 -10점</label>
        <br>
      </div><br />
      <!-- 2. 외모 -->
      <div>
        <p style="font-size: 20px;">2. 외모</p>
        <input name="product" id="check2" value="30" type="checkbox" onclick="totalIt()" /><label for="check2">1) 머리부터 발 끝까지 이상형 -30점</label>
        <br>
        <input name="product" id="check2" value="25" type="checkbox" onclick="totalIt()" /><label for="check2">2) 모든것이 평타 혹은 그 이상 -25점</label>
        <br>
        <input name="product" id="check2" value="20" type="checkbox" onclick="totalIt()" /><label for="check2">3) 평범하고 수수한 외형 -20점</label>
        <br>
        <input name="product" id="check2" value="15" type="checkbox" onclick="totalIt()" /><label for="check2">4) 내 취향은 아님 -15점</label>
        <br>
        <input name="product" id="check2" value="10" type="checkbox" onclick="totalIt()" /><label for="check2">5) 모두의 취향이 아님 -10점</label>
        <br>
      </div><br />
      <!-- 3. 성격 -->
      <div>
        <p style="font-size: 20px;">3. 성격</p>
        <input name="product" id="check3" value="30" type="checkbox" onclick="totalIt()" /><label for="check3">1) 배려심 킹 -30점</label>
        <br>
        <input name="product" id="check3" value="25" type="checkbox" onclick="totalIt()" /><label for="check3">2) 남자친구를 엄청 신뢰해서 방관함 -25점</label>
        <br>
        <input name="product" id="check3" value="20" type="checkbox" onclick="totalIt()" /><label for="check3">3) 모든게 둥글지만 다소 자기중심적 -20점</label>
        <br>
        <input name="product" id="check3" value="15" type="checkbox" onclick="totalIt()" /><label for="check3">4) 연인에 대해 심한 집착 보유자(친구들과 약속에도 영상통화 필수) -15점</label>
        <br>
        <input name="product" id="check3" value="10" type="checkbox" onclick="totalIt()" /><label for="check3">5) 화가나면 자신의 감정을 주체 못하는 다혈질 (습관적으로 이별을 말함) -10점</label>
        <br>
      </div><br />
      <!-- 4. 대화 -->
      <div>
        <p style="font-size: 20px;">4. 대화</p>
        <input name="product" id="check4" value="30" type="checkbox" onclick="totalIt()" /><label for="check4">1) 티키타카 환상 ("아!" 하면 "예!"가 나오는 수준) -30점</label>
        <br>
        <input name="product" id="check4" value="25" type="checkbox" onclick="totalIt()" /><label for="check4">2) 대화의 칭찬을 곁들여 자존심을 올려주는 사람 -25점</label>
        <br>
        <input name="product" id="check4" value="20" type="checkbox" onclick="totalIt()" /><label for="check4">3) 대화의 물꼬를 트는데까지는 어렵지만 입 터지면 장난 아님 -20점</label>
        <br>
        <input name="product" id="check4" value="15" type="checkbox" onclick="totalIt()" /><label for="check4">4) 맥커터 ex) 남들 다 베트맨 얘기하는데 지 혼자 스파이더맨 얘기함 -15점</label>
        <br>
        <input name="product" id="check4" value="10" type="checkbox" onclick="totalIt()" /><label for="check4">5) 남자의 자존심을 건드리는 화법 ex)비교, 꼴랑 그거 하는데 시간이 그렇게 걸린다고? -10점</label>
        <br>
      </div><br />
      <!-- 5. 친구 -->
      <div>
        <p style="font-size: 20px;">5. 종교</p>
        <input name="product" id="check5" value="30" type="checkbox" onclick="totalIt()" /><label for="check5">1) 나와 같은 종교 OR 무교 -30점</label>
        <br>
        <input name="product" id="check5" value="25" type="checkbox" onclick="totalIt()" /><label for="check5">2) 나와 종교가 다르지만 노터치 -25점</label>
        <br>
        <input name="product" id="check5" value="20" type="checkbox" onclick="totalIt()" /><label for="check5">3) 주 1회 종교활동 -20점</label>
        <br>
        <input name="product" id="check5" value="15" type="checkbox" onclick="totalIt()" /><label for="check5">4) 처가될 집안이 우리 집안과 종교가 다름 (전도강요) -15점</label>
        <br>
        <input name="product" id="check5" value="10" type="checkbox" onclick="totalIt()" /><label for="check5">5) 생활에 지장있는 정도의 종교활동 OR 사이비 종교 -10점</label>
        <br>
      </div><br />
